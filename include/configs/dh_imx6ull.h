...
#define EXTRA_ENV_SETTINGS \
	...
	"linuxargs=setenv bootargs" \
		...
		" dhblloc=${dhblloc} dhenvloc=${dhenvloc} dhcor=${dhcor} dhsw=${dhsw} maveo_hw=${maveo_hw} SN=${SN} PSN=${PSN}\0" \
	...
	"setupdateargs=setenv bootargs" \
		...
		" vt.global_cursor_default=0 consoleblank=0 ${backlight} dhblloc=${dhblloc} dhenvloc=${dhenvloc} dhcor=${dhcor} dhsw=${dhsw} maveo_hw=${maveo_hw} SN=${SN} PSN=${PSN}\0" \
...
#define EXTRA_BOOT_ENV_FOR_USB \
	...
	"load_bootscript_alt=echo Loading ${bootscript_file_alt}...;" \
		" load usb ${usbdev}:${usbpart} ${loadaddr} ${bootscript_file_alt}\0" \
	"load_fdt=echo Loading device tree ${fdt_file}...;" \
